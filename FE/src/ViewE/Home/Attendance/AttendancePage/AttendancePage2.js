...
const AttendancePage2 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (employee_id) {
          const response = await fetch(`${BASE_URL}/informationemployeeattendance?employee_id=${employee_id}`);
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      if (employee_id) {
        const response = await fetch(`${BASE_URL}/Updateattendancetohalf`, {
          method: 'PUT',
          headers: {
